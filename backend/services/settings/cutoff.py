#!/usr/bin/env python3
"""
Cutoff Settings Manager for business domain settings.

Manages cutoff times for EPCs, Clients, and Units in SQLite database.
...
from backend.services.data.db_manager.db_schema import get_db_connection, init_database, create_default_stratcon_epc
...
class CutoffSettingsManager:
    """
    Manages cutoff settings for clients, tenants, and units in the database.
    
    Handles the hierarchy: Unit → Client → EPC → System Default
    """
    
    def __init__(self, logger: Optional[ReportLogger] = None):
        """
        Initialize cutoff settings manager.
    ...
    def _find_or_create_epc(self, epc_name: str, conn: sqlite3.Connection) -> int:
    ...
    def _find_unit_by_identifier(
        ...
        cursor.execute("""
            ...
            JOIN unit_tenants_history uth ON u.id = uth.unit_id
    ...
    def get_cutoff_datetime(
        ...
        try:
            # Get client ID
            client_id = self._find_or_create_client(client_token, conn=conn)
            
            # Try to find unit if tenant_token provided
            unit_id = None
            if tenant_token:
                unit_id = self._find_unit_by_identifier(client_id, tenant_token, conn)
                if unit_id:
                    ...
                    if row and row['cutoff_day'] is not None:
                        ...
                        self.logger.debug(f"Found unit-level settings for unit_id={unit_id} (client={client_token}, tenant={tenant_token})")
            ...
            if cutoff_day is None:
                ...
                if row and row['cutoff_day'] is not None:
                    ...
                    cutoff_minute = row['cutoff_minute'] if row['cutoff_minute'] is not None else DEFAULT_CUTOFF_MINUTE
                    cutoff_second = row['cutoff_second'] if row['cutoff_second'] is not None else DEFAULT_CUTOFF_SECOND
                    self.logger.debug(f"Found client-level settings for {client_token}")
            ...
            if cutoff_day is None:
                ...
                if row and row['cutoff_day'] is not None:
                    ...
                    cutoff_minute = row['cutoff_minute'] if row['cutoff_minute'] is not None else DEFAULT_CUTOFF_MINUTE
                    cutoff_second = row['cutoff_second'] if row['cutoff_second'] is not None else DEFAULT_CUTOFF_SECOND
                    self.logger.debug(f"Found EPC-level settings for {client_token}")
            
            # If still no cutoff_day, return None (will fall back to CSV or default)
            if cutoff_day is None:
                self.logger.debug(f"No cutoff settings found in database for {client_token}, returning None")
                return None
            ...
            self.logger.debug(f"Created cutoff datetime: {cutoff_datetime} (day={cutoff_day}, time={cutoff_hour:02d}:{cutoff_minute:02d}:{cutoff_second:02d})")
    ...
    def set_client_cutoff(
        ...
        try:
            # Get or create EPC
            if epc_name:
                epc_id = self._find_or_create_epc(epc_name, conn)
            else:
                cursor.execute("SELECT id FROM epcs WHERE name = 'Stratcon'")
                row = cursor.fetchone()
                if row:
            ...
            client_id = self._find_or_create_client(client_token, epc_id, conn)
    ...
    def set_tenant_cutoff(
        ...
        try:
            ...
            if unit_id:
                ...
                self.logger.warning(f"⚠️  Unit not found for client={client_token}, tenant={tenant_token}. Unit must be created via full schema.")
